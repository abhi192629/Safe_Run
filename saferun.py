...
class Cmd:
    ...
    resolved_targets: List[str] # normalized absolute paths 
    has_pipe: bool
    has_sudo: bool
...
class Const:
    """Single place for engine constants (refactor-only, no behavior changes)."""
    SYS_DIRS = frozenset([
        "/", "/bin", "/sbin", "/lib", "/lib64",
    ...
    # Tutorial placeholder: /dev/sdX, /dev/sdY — single uppercase, no digits
    # These appear in documentation but are NOT real devices
    DEV_PLACEHOLDER = re.compile(r'^/dev/sd[A-Z]$')
    # Paths that are generally safe to delete recursively
    SAFE_PATHS = frozenset(["/tmp", "/var/tmp", "/var/cache", "/var/log"])
    # Precompiled regexes
    ECHO_CMD = re.compile(r'^(?:echo|printf)\s')
    PIPE_MATCH = re.compile(r'\|')
    ROOT_WILDCARD = re.compile(r'^/\*')
    VAR_EXPANSION = re.compile(r'^\$\{?(?:HOME|PWD|OLDPWD|USER)\}?$')
    REDIRECT_SYS = re.compile(r'>\s*/(?:etc|dev)/')
    PIPE_TO_SHELL = re.compile(
        r"\|\s*(?:sudo\s+)?(?:/usr/bin/|/bin/)?(?:bash|sh|dash|zsh|fish)\b",
        re.IGNORECASE
    )
...
class Parser:
    ...
    # Aliases into Const so the rules engine has one “source of truth”.
    SYS_DIRS = Const.SYS_DIRS
    DEV_DISK = Const.DEV_DISK
    DEV_PLACEHOLDER = Const.DEV_PLACEHOLDER
    SAFE_PATHS = Const.SAFE_PATHS
    ...
    def parse(cls, raw: str) -> Cmd:
        ...
        has_pipe = bool(Const.PIPE_MATCH.search(norm))

        # ── Fix 2: Detect echo/printf context (false positive prevention)
        is_echo = bool(Const.ECHO_CMD.match(norm))
        ...
        import os
        resolved_targets = [os.path.normpath(t) for t in targets]
        return Cmd(raw, norm, binary, flags, args, targets, resolved_targets,
    ...
    def targets_system(cls, c: Cmd) -> bool:
        return any(
            ...
            or Const.ROOT_WILDCARD.match(t) or cls.DEV_DISK.match(t)
            for t in c.targets
        )
    ...
    def targets_root_resolved(cls, c: Cmd) -> bool:
        """Check if any target resolves to / even via relative paths or wildcards."""
        for t, resolved in zip(c.targets, c.resolved_targets):
            # Wildcard on root or resolved root: /tmp/../* → resolves to /*
            if resolved in cls.SYS_DIRS or resolved == "/":
                return True
            # Wildcard that resolves to root level: /tmp/../* or /*
            if "*" in t:
                import os as _os
                base = _os.path.normpath(t.split("*")[0].rstrip("/") or "/")
                if base in cls.SYS_DIRS or base == "/":
                    return True
            # Variable expansion: $HOME, $PWD, ${HOME}, ${PWD}
            if Const.VAR_EXPANSION.match(t):
                return True
        return False
...
def highlight_danger(cmd: str, parts: tuple) -> str:
    ...
    for part in parts:
        ...
        if idx != -1:
            styled = f"{C.RED}{C.BOLD}{part}{C.RESET}{C.CYAN}"
            result = result[:idx] + styled + result[idx+len(part):]
    return f"{C.CYAN}{result}{C.RESET}"

def hit(*labels: str) -> tuple:
    """Standard detector return shape: one or more string labels/parts."""
    return tuple(labels)
...
def _rm_root(c: Cmd) -> Optional[tuple]:
    """rm recursively targeting / or root wildcard, including /tmp/../* style paths."""
    if not is_rm(c): return None
    if not P.is_recursive(c): return None
    import os as _os
    for a in c.args:
        # Direct root targets
        if a in ("/", "/*") or Const.ROOT_WILDCARD.match(a):
            return hit("-r or -R or --recursive", a)
        # Relative path resolving to /
        resolved = P.resolve_path(a)
        if resolved == "/":
            return hit("-r or -R or --recursive", a)
        # Wildcard where base resolves to root: /tmp/../* → base=/tmp/.. → /
        if "*" in a:
            base = _os.path.normpath(a.split("*")[0].rstrip("/") or "/")
            if base == "/" or base in P.SYS_DIRS:
                return hit("-r or -R with wildcard", a)
    return None
...
def _rm_sysdir(c: Cmd) -> Optional[tuple]:
    ...
    if P.targets_system(c):
        ...
        return hit("-r or -R", bad[0] if bad else "system path")
    return None

def _rm_wildcard(c: Cmd) -> Optional[tuple]:
    if not is_rm(c): return None
    for a in c.args:
        if Const.ROOT_WILDCARD.match(a):
            return hit("/*", a)
    return None

def _dd_disk(c: Cmd) -> Optional[tuple]:
    """CRITICAL: dd targeting a real disk device."""
    if c.binary != "dd": return None
    for a in c.args:
        if a.startswith("of="):
            dev = a.split("=", 1)[-1]
            if P.DEV_PLACEHOLDER.match(dev): return None  # placeholder — handled separately
            if P.DEV_DISK.match(dev):
                return hit("of=", dev)
    return None

def _dd_placeholder(c: Cmd) -> Optional[tuple]:
    """MEDIUM: dd targeting a tutorial placeholder like /dev/sdX."""
    if c.binary != "dd": return None
    for a in c.args:
        if a.startswith("of="):
            dev = a.split("=", 1)[-1]
            if P.DEV_PLACEHOLDER.match(dev):
                return hit(dev, "looks like a tutorial placeholder — verify it is a real device")
    return None

def _mkfs(c: Cmd) -> Optional[tuple]:
    """CRITICAL: mkfs targeting a real block device."""
    if not c.binary.startswith("mkfs"): return None
    for t in c.targets:
        if P.DEV_PLACEHOLDER.match(t): return None  # placeholder — handled separately
        if P.DEV_DISK.match(t):
            return hit(c.binary, t)
    return None

def _mkfs_placeholder(c: Cmd) -> Optional[tuple]:
    """MEDIUM: mkfs targeting a tutorial placeholder like /dev/sdX."""
    if not c.binary.startswith("mkfs"): return None
    for t in c.targets:
        if P.DEV_PLACEHOLDER.match(t):
            return hit(t, "looks like a tutorial placeholder — verify it is a real device")
    return None
...
def _chmodR_sys(c: Cmd) -> Optional[tuple]:
    if c.binary == "chmod" and P.is_recursive(c) and P.targets_system(c):
        return hit("-R on system path")
    return None

def _kill1(c: Cmd) -> Optional[tuple]:
    if c.binary == "kill":
        # Direct signal flags: -9, -SIGKILL
        direct_kill = P.flag(c, "-9", "-SIGKILL")
        # Signal name via -s KILL or --signal KILL
        sig_flag = P.flag(c, "-s", "--signal")
        sig_name = sig_flag and any(a in ("KILL", "SIGKILL", "9") for a in c.args)
        is_kill_signal = direct_kill or sig_name

        if is_kill_signal:
            if "1" in c.args:
                return hit("-9 or -s KILL", "PID 1")
            if "-1" in c.flags:
                return hit("-9 or -s KILL", "-1 (kills ALL processes)")
    if c.binary == "killall" and any(a in ("systemd", "init") for a in c.args):
        return hit("systemd/init")
    return None
...
def _pipe_exec(c: Cmd) -> Optional[tuple]:
    """curl/wget piped to a shell — not arbitrary pipes like | less."""
    if c.binary not in ("curl", "wget") or not c.has_pipe:
        return None
    match = Const.PIPE_TO_SHELL.search(c.norm)
    if match:
        return hit(" ".join(match.group(0).split()))
    return None

def _root_shell(c: Cmd) -> Optional[tuple]:
    if c.has_sudo and c.binary in ("bash","sh","zsh","fish","su","csh","ksh"):
        return hit(f"sudo {c.binary}")
    return None

def _ip_forward(c: Cmd) -> Optional[tuple]:
    if c.binary == "sysctl" and any("ip_forward" in a and "1" in a for a in c.args):
        return hit("ip_forward=1")
    return None
...
def _apt_critical(c: Cmd) -> Optional[tuple]:
    ...
    return hit(bad) if bad else None
...
def _history_exec(c: Cmd) -> Optional[tuple]:
    ...
    if c.binary == "history":
        match = Const.PIPE_TO_SHELL.search(c.norm)
        if match:
            return hit("history", " ".join(match.group(0).split()))
    return None
...
def _rm_config(c: Cmd) -> Optional[tuple]:
    ...
    if any(t.startswith("~/.") and t not in ("~/.bashrc","~/.zshrc","~/.profile","~/.bash_profile") for t in c.targets):
        return hit("user config directory")
    return None
...
RULES = [
    ...
    rule("CRITICAL", "Fork bomb detected",
         ...
         pattern=r'([a-zA-Z0-9_:-]+)\s*\(\s*\)\s*\{\s*.*\1\s*\|\s*\1\s*&.*\}'),
    ...
    rule("MEDIUM", "Formatting a placeholder device name",
         "The device /dev/sdX looks like a tutorial placeholder, not a real disk. Verify before running.",
         ("If this IS a real device, formatting it will erase all data permanently",
          "Tutorial placeholders like /dev/sdX are not real — running this does nothing or errors"),
         "Replace sdX with your actual device. Run lsblk to find real device names.",
         detector=_mkfs_placeholder),

    rule("MEDIUM", "Writing raw data to a placeholder device name",
         "The device /dev/sdX looks like a tutorial placeholder, not a real disk.",
         ("If this IS a real device, writing to it will destroy all data permanently",
          "Tutorial placeholders are not real — check your actual device with lsblk"),
         "Replace sdX with your actual device. Confirm with: lsblk or fdisk -l",
         detector=_dd_placeholder),
...
# ── Precomputed rule lists — grouped by binary for faster lookup ─────
# Detectors split by binary: only check rm rules when cmd is rm, etc.
# Falls back to _GENERIC for commands without specific grouping
DETECTOR_RULES = [r for r in RULES if r["detector"]]
PATTERN_RULES  = [r for r in RULES if r["pattern"]]

# Group detector rules by binary name for O(1) lookup
_BINARY_GROUPS: dict = {}
for _r in DETECTOR_RULES:
    # Extract binary hint from detector function name
    _fname = _r["detector"].__name__
    _bins = {
        "_rm":       "rm",
        "_dd":       "dd",
        "_mkfs":     "mkfs",
    ...
    _bin = None
    for prefix, binary in _bins.items():
        if _fname.startswith(prefix):
            _bin = binary
            break
    key = _bin if _bin else "__any__"
    _BINARY_GROUPS.setdefault(key, []).append(_r)

def get_rules_for(binary: str) -> list:
    """Wrap binary-grouped rule lookup (hide engine internals)."""
    return _BINARY_GROUPS.get(binary, DETECTOR_RULES)
...
def analyse(cmd: str):
    ...
    if _skip(c):
        return None
    return evaluate_rules(cmd, c)

def get_detector_rules_for_cmd(c: Cmd) -> list:
    """Compute which semantic detector rules to run for this parsed command."""
    # Resolve binary key — handle mkfs.ext4 → mkfs
    binary_key = c.binary
    if binary_key.startswith("mkfs"):
        binary_key = "mkfs"

    # When sudo is present check BOTH the actual binary AND the sudo group
    if c.has_sudo:
        seen = set()
        rules_to_check = []
        for _rule in (_BINARY_GROUPS.get("sudo", []) +
                      _BINARY_GROUPS.get(binary_key, []) +
                      _BINARY_GROUPS.get("__any__", [])):
            if id(_rule) not in seen:
                seen.add(id(_rule))
                rules_to_check.append(_rule)
        rules_to_check = rules_to_check or get_rules_for(binary_key) or DETECTOR_RULES
...
def evaluate_rules(cmd: str, c: Cmd) -> Optional[Profile]:
    """
    Central rule-evaluation pipeline:
    - Semantic detector rules first
    - Regex pattern rules second
    """
    ...
    if not c.is_echo:
        for r in get_detector_rules_for_cmd(c):
            danger = r["detector"](c)
    ...
    pattern_target = cmd
    if c.is_echo:
        if not Const.REDIRECT_SYS.search(cmd):
...
LOG_PATH  = os.path.join(HOME, ".saferun.log")
...
def run_cli(argv: Optional[List[str]] = None) -> int:
    ...
    if not args or not any(a.strip() for a in args):
        print(f"{C.RED}Error: No valid command provided.{C.RESET}"); return 1

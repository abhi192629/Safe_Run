...
# curl/wget piped directly into an interactive shell (not e.g. | less)
...
def _pipe_exec(c: Cmd) -> Optional[tuple]:
    ...
    if c.binary not in ("curl", "wget") or not c.has_pipe:
        return None
    if _PIPE_TO_SHELL.search(c.norm):
        return hit("| bash/sh")
    return None
...
# wget shares pipe exec detection with curl
if "curl" in _BINARY_GROUPS:
    _BINARY_GROUPS.setdefault("wget", []).extend(_BINARY_GROUPS["curl"])
